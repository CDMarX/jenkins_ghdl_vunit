...
RUN echo '# deb http://snapshot.debian.org/archive/debian-security/20190610T000000Z buster/updates main' >> /etc/apt/sources.list
RUN echo 'deb http://security.debian.org/debian-security buster/updates main' >> /etc/apt/sources.list
RUN echo '# deb http://snapshot.debian.org/archive/debian/20190610T000000Z buster-updates main' >> /etc/apt/sources.list
RUN echo 'deb http://deb.debian.org/debian buster-updates main' >> /etc/apt/sources.list

USER ${user}
RUN apt-get update -qq && apt-get -y install libgnat-8 gcc libc6-dev curl
RUN apt-get autoclean -y && apt-get clean -y && apt-get autoremove -y

COPY --from=buster-mcode ./*.tgz /tmp/

RUN GHDL_TGZ=`ls /tmp/ghdl-*` \
 && tar -xzf "$GHDL_TGZ" -C /usr/local \
 && rm -f "$GHDL_TGZ"

#---

FROM do-ghdl AS do_python

RUN apt-get -y install python3 python3-pip

#---

FROM do_python AS do_vunit

RUN pip3 install vunit_hdl

#---

FROM do_vunit AS do-jenkins
